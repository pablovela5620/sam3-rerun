...
class SAM3DBodyEstimator:
    def __init__(
        ...
        self.faces: np.ndarray = self.model.head_pose.faces.cpu().numpy()
    ...
    def process_one_image(
        ...
        if isinstance(img, str):
            img_array: UInt8[np.ndarray, "h w 3"] = load_image(
                img, backend="cv2", image_format="bgr"
            ).astype(np.uint8)
            image_format: Literal["bgr", "rgb"] = "bgr"
        else:
            print("####### Please make sure the input image is in RGB format")
            img_array = cast(UInt8[np.ndarray, "h w 3"], img)
            image_format = "rgb"
        height: int
        width: int
        ...
        masks_score: Float[np.ndarray, "n"] | None = None
        ...
        if cam_int is not None:
            ...
            input_image: np.ndarray = cast(list[NoCollate], batch["img_ori"])[0].data
            cam_int_pred: torch.Tensor | np.ndarray = self.fov_estimator.get_cam_intrinsics(input_image)
            if isinstance(cam_int_pred, np.ndarray):
        ...
        outputs: Any = self.model.run_inference(
            img_array,
            batch,
        ...
        pose_output_dict: dict[str, Any] = cast(dict[str, Any], pose_output)
        out: dict[str, Any] = cast(dict[str, Any], pose_output_dict["mhr"])
        out_np: dict[str, np.ndarray] = cast(
            dict[str, np.ndarray], recursive_to(recursive_to(out, "cpu"), "numpy")
        )
