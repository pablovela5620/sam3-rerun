...
class Renderer:
    ...
    def __init__(
        self,
        focal_length: float | np.floating,
        faces: Optional[Int[np.ndarray, "faces 3"]] = None,
    ):
        """
        ...
        self.focal_length: float | np.floating = focal_length
        self.faces: Optional[Int[np.ndarray, "faces 3"]] = faces

    def __call__(
        self,
        vertices: Float[np.ndarray, "verts 3"],
        cam_t: Float[np.ndarray, "3"],
        image: Float[np.ndarray, "h w 3"] | UInt8[np.ndarray, "h w 3"],
        ...
        camera_translation: Float[np.ndarray, "3"] = cam_t.copy()
        ...
        color: Float[np.ndarray, "h w 4"] = color.astype(np.float32) / 255.0
        renderer.delete()

        if return_rgba:
            return color

        valid_mask: Float[np.ndarray, "h w 1"] = (color[:, :, -1])[:, :, np.newaxis]
        output_img: Float[np.ndarray, "h w 3"] = (
            color[:, :, :3] * valid_mask + (1 - valid_mask) * image
        )
    ...
    def render_rgba(
        self,
        vertices: Float[np.ndarray, "verts 3"],
        cam_t: Optional[Float[np.ndarray, "3"]] = None,
        rot=None,
        rot_axis=[1, 0, 0],
        rot_angle=0,
        camera_z=3,
        ...
        if cam_t is not None:
            ...
            camera_translation: Float[np.ndarray, "3"] = np.array(
                [0, 0, camera_z * self.focal_length / render_res[1]], dtype=np.float32
            )
    ...
    def render_rgba_multiple(
        self,
        vertices: List[Float[np.ndarray, "verts 3"]],
        cam_t: List[Float[np.ndarray, "3"]],
        rot_axis=[1, 0, 0],
        rot_angle=0,
        mesh_base_color=(1.0, 1.0, 0.9),
        scene_bg_color=(0, 0, 0),
